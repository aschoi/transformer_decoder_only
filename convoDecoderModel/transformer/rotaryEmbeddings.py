...
class RotaryEmbedding(nn.Module):
    ...
    def forward(
        ...
        req_seq_len = position_offset + seq_len

        if req_seq_len > self.max_seq_len:
            tnsr_cos, tnsr_sin = self._build_cache(req_seq_len)
            self.tnsr_cos_cached = tnsr_cos
            self.tnsr_sin_cached = tnsr_sin
            self.max_seq_len = req_seq_len

        end_position = position_offset + seq_len

        return (
            self.tnsr_cos_cached[position_offset:end_position].to(device=device, dtype=dtype),
            self.tnsr_sin_cached[position_offset:end_position].to(device=device, dtype=dtype)
        )
