...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.d_k = d_model // num_attn_heads   # d_k === head_dim 
    ...
    def forward(
        ...
        if self.training and kv_cache is not None:
            raise RuntimeError("KV caching is for inference only. Cannot be used in conjuction with trainging")

        if kv_cache:  # means we're in inference mode
        
            batch_size, seq_len, _ = tnsr_X.shape

            tnsr_Q = self.nn_linearProj_w_Q(tnsr_X)
            tnsr_K = self.nn_linearProj_w_K(tnsr_X)
            tnsr_V = self.nn_linearProj_w_V(tnsr_X)

            # reshape:  (B, T, D) --> (B, H, T, Dh)
            tnsr_Q = tnsr_Q.view(batch_size, seq_len, self.num_attn_heads, self.d_k).transpose(1, 2)
            tnsr_K = tnsr_K.view(batch_size, seq_len, self.num_attn_heads, self.d_k).transpose(1, 2)
            tnsr_V = tnsr_V.view(batch_size, seq_len, self.num_attn_heads, self.d_k).transpose(1, 2)

            position_offset = kv_cache.seq_len  # === query_start

            tnsr_cos, tnsr_sin = self.rotary_emb(seq_len, tnsr_X.device, tnsr_X.dtype, position_offset=position_offset)
            tnsr_Q, tnsr_K = apply_rotary_pos_emb(tnsr_Q, tnsr_K, tnsr_cos, tnsr_sin)


            tnsr_K, tnsr_V = kv_cache.update(tnsr_K, tnsr_V)
            key_length = kv_cache.seq_len

            if seq_len == 1:
                causal_mask = None
            else:
                causal_mask = build_cached_causal_mask(
            ...
            tnsr_attn_output = self.nn_linearProj_w_output(tnsr_attn_output)
            

            return tnsr_attn_output, None
...
def build_cached_causal_mask(
    *,
    ...
    """
    Build a causal mask using absolute sequence positions
    Args:


    Return:
                    <tensor<bool>>      shape:  (1, 1, query_length, key_length)
