...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.sublayer1_attention = CausalSelfAttention(
            d_model, num_attn_heads, max_seq_len, rope_baseFreq, dropout_rate
        )
    ...
    def forward(
        ...
        tnsr_attn_output, tnsr_attn_weights = self.sublayer1_attention(
            self.sublayer1_norm(tnsr_X), 
            tnsr_padding_mask, 
            return_attn_weights
        )
        ...
        tnsr_ff_output = self.sublayer2_ff(self.sublayer2_norm(tnsr_X))
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for decoderLayer in self.stack_transformer_layers:

            tnsr_X, tnsr_attn_weights = decoderLayer(tnsr_X, tnsr_padding_mask, return_attn_weights)
